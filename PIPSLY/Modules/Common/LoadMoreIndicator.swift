...
class LoadMoreActivityIndicator {
    ...
    init (tableView: UITableView, spacingFromLastCell: CGFloat, spacingFromLastCellWhenLoadMoreActionStart: CGFloat, color: UIColor? = .black) {
    ...
    private var isHidden: Bool {
        if tableView.contentSize.height < tableView.frame.size.height - 100 {
            return true
        } else {
            return false
        }
    ...
    func scrollViewDidScroll(scrollView: UIScrollView, loadMoreAction: () -> Void) {
        let offsetMargin: CGFloat = 50
        let offsetY = scrollView.contentOffset.y + offsetMargin
        activityIndicatorView.isHidden = isHidden
