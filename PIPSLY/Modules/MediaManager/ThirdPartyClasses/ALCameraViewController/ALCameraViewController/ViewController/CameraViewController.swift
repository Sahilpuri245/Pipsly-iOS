...
open class CameraViewController: UIViewController {
    ...
    public init(croppingEnabled: Bool, allowsLibraryAccess: Bool = true, useImage: Bool, completion: @escaping CameraViewCompletion) {
        super.init(nibName: nil, bundle: nil)
        onCompletion = completion
        allowCropping = croppingEnabled
        cameraOverlay.isHidden = !allowCropping
    ...
    private func checkPermissions() {
        ...
        } else if self.usePhoto == true && self.useImage == false {
            let authorizationStatus: PHAuthorizationStatus = AppAccessCheck.photoLibraryAllowAccessToApplication()
   ...
   class AppAccessCheck: NSObject {

        // MARK: - CAMERA ACCESS CHECK
        class func cameraAllowsAccessToApplication() -> AVAuthorizationStatus {
            let authorizationStatus = AVCaptureDevice.authorizationStatus(for: AVMediaType.video)
            return authorizationStatus
        }
        // MARK: - PHOTO LIBRARY ACCESS CHECK
        class func photoLibraryAllowAccessToApplication() -> PHAuthorizationStatus {
    ...
    internal func capturePhoto() {
        ...
        if connection.isEnabled {
            ...
            cameraView.capturePhoto { [weak self] image in
                ...
                if self?.useImage == true {
                    self?.layoutCameraResultAsImage(image: image)
                } else {
    ...
    internal func layoutCameraResultAsImage(image: UIImage) {
        cameraView.stopSession()
        startConfirmControllerWithImage(image: image)
        toggleButtons(enabled: true)
    }
    ...
    private func startConfirmControllerWithImage(image: UIImage) {
        ...
        confirmViewController.onComplete = { [weak self] image, asset in
            ...
            if self?.useImage == true && image != nil {
                self?.onCompletion?(image, asset)
                self?.onCompletion = nil
            } else {
                guard let image = image, let asset = asset else {
